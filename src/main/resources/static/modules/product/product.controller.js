(function () {
  ...
  function ProductController($scope, $http, $window, $timeout, $location, $state, $stateParams, API, ENUM, AppAuthenticationService, ConfirmProductService, CartModel) {
      ...
      var goodsId = $stateParams.product;
      ...
      function _reloadCartGoodsCount(){
          API.cart.goodsCount().then(function (res) {
                  if (res) {
                      console.log(res);
                      $scope.cartModel.total = res.data.cartTotal.goodsCount;
                  }
              });
      }

      function _reloadGoodsDetail(id) {
          API.goods
              .detail({
                  id: id
              })
              .then(function (res) {
                  if(res){
                      ...
                      $scope.goods = res.data.info;
                      $scope.galleries = res.data.gallery;
                      $scope.attributes = res.data.attribute;
                      $scope.issueList = res.data.issue;
                      $scope.comments = res.data.comment;
                      $scope.brand = res.data.brand;
                      $scope.specifications = res.data.specificationList;
                      $scope.productList = res.data.productList;
                      $scope.userHasCollect = res.data.userHasCollect;
                  }
              });
      }

      function _reload() {
          _reloadGoodsDetail(goodsId);
          _reloadCartGoodsCount();
      }
