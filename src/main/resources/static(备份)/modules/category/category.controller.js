(function () {
	...
	function CategoryController($scope, $http, $location, $state, API, ENUM, CartModel) {
        ...
        function _reloadCategoryGoods() {
            API.category
                .categoryList()
                .then(function (categoryGoods) {
                    if (categoryGoods && categoryGoods.length) {
                        $scope.categoryGoods = categoryGoods;
                    } else {
                        $scope.categoryGoods = null;
                    }
                });
        }
