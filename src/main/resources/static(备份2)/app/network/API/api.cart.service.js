(function () {
    ...
    function APICartService($http, $q, $timeout, CacheFactory, ENUM) {
        ...
        service.goodsCount = _goodsCount;

        return service;

        function _get( params ) {
            return this.fetch( '/cart/index', params, false, function(res){
                return ENUM.ERROR_CODE.OK == res.data.errno ? res.data : null;
            });
        }

        function _add( params ) {
            return this.fetch( '/cart/add', params, false, function(res){
                return ENUM.ERROR_CODE.OK == res.data.error_code ? true : false;
            });
        }

        function _delete( params ) {
            return this.fetch( '/cart/delete', params, false, function(res){
                return ENUM.ERROR_CODE.OK == res.data.error_code ? true : false;
            });
        }

        function _update( params ) {
            return this.fetch( '/cart/update', params, false, function(res){
                return ENUM.ERROR_CODE.OK == res.data.error_code ? true : false;
            });
        }
        ...
        function _checkout( params ) {
            return this.fetch( '/cart/checked', params, false, function(res){
                return ENUM.ERROR_CODE.OK == res.data.error_code ? res.data.order : null;
            });
        }

        function _goodsCount( params ) {
            return this.fetch( '/cart/goodscount', params, false, function(res){
                return ENUM.ERROR_CODE.OK == res.data.errno ? res.data : null;
            });
        }
